...
class TicketTranscriptListener : Extension() {
    ...
    override suspend fun setup() {
        event<GuildButtonInteractionCreateEvent> {
            ...
            action {
                ...
                TicketSystem.logTicketAction(event.interaction.guild, ticket) {
                    description = "Transcript for ticket #${ticket.id} manually requested by ${event.interaction.user.mention}."
                }
    ...
    companion object {
        ...
        private suspend fun generateTranscriptEmbed(placeholders: TicketPlaceholders, logChannel: Boolean): EmbedBuilder {
            ...
            val ticketUser = placeholders.ticketUser.await() ?: placeholders.interactionUser
            return buildEmbed {
                if(logChannel) {
                    description = "Transcript generated by ${placeholders.interactionUser.mention}"

                    field("Ticket User", true) {
                        "<@${placeholders.ticketUserId}>"
                    }
                }

                author {
                    name = ticketUser.effectiveName
                    icon = (ticketUser.avatar ?: ticketUser.defaultAvatar).cdnUrl.toUrl {
