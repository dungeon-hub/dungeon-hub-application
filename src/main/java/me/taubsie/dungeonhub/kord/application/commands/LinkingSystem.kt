    ...
    override suspend fun setup() {
        publicSlashCommand(::LinkArguments) {
            ...
            action {
                respond {
                    val linkedTo =
                        DiscordUserConnection.getInstance().getById(user.id.value.toLong()).map { it.minecraftId }
                    ...
                    embed.description =
                        "You're now linked to `${MojangConnection.getInstance().getNameByUUID(linkedId)}`."
        ...
        publicSlashCommand {
            ...
            action {
                respond {
                    val userModel: DiscordUserModel? = DiscordUserConnection.getInstance()
        ...
        publicSlashCommand {
            ...
            action {
                respond {
                    val oldUserModel = DiscordUserConnection.getInstance()
                    ...
                    embed.description = "Unlinked successfully from account `${
                        MojangConnection.getInstance().getNameByUUID(oldUserModel.minecraftId)
                    }`."
                    embed.color = EmbedColor.POSITIVE.color
        ...
        event<GuildButtonInteractionCreateEvent> {
            ...
            action {
                when (event.interaction.componentId) {
                    "link_user" -> {
                        val linkedTo = DiscordUserConnection.getInstance()
