...
object RolesService {
    ...
    suspend fun calculateRoles(member: Member): List<Role> {
        val serverRoles = DiscordRoleConnection.getInstance(member.guildId.value.toLong())
            .allRoles
            .orElse(ArrayList())
            .stream()
            .collect(
        ...
        val isVerified = DiscordUserConnection.getInstance().getLinkedById(member.id.value.toLong()).isPresent
    ...
    fun applyRoleGroups(server: GuildBehavior, roles: MutableSet<Snowflake>): MutableSet<Snowflake> {
        ...
        val roleGroups = DiscordRoleGroupConnection.getInstance(server.id.value.toLong())
            .all
            .orElse(ArrayList())
