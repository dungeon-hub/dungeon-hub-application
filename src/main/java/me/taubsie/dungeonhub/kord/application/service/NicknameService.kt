...
import me.taubsie.dungeonhub.kord.application.misc.PlayerInformation
...
object NicknameService {
    ...
    fun linkToIgn(ign: String?, user: User): UUID {
     ...
     """.trimIndent()
        ...
        val userModel = DiscordUserConnection.getInstance().updateUser(user.id.value.toLong(), updateModel)
    ...
    suspend fun updateNickname(member: Member, serverRoles: List<Role>?) {
        val discordUserModel = DiscordUserConnection.getInstance().getById(member.id.value.toLong())
