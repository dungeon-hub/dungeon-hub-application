...
enum class FlaggingApi(
    ...
    JERRY(
        "Jerry",
        { uuid: UUID? -> FlaggingConnection.getInstance().isJerryFlagged(uuid).orElse(null) },
        { discordId: Long? -> FlaggingConnection.getInstance().isJerryFlagged(discordId).orElse(null) }
    ),
    HYPIXEL_SAFETY(
        "Hypixel Safety",
        { uuid: UUID? -> FlaggingConnection.getInstance().isSafetyFlagged(uuid).orElse(null) },
        { discordId: Long? -> FlaggingConnection.getInstance().isSafetyFlagged(discordId).orElse(null) }
    );
