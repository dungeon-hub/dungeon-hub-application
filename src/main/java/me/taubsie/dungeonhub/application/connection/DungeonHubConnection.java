...
public class DungeonHubConnection {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.isSuccessful()) {
                ...
                return DungeonHubService.getInstance().getGson().fromJson(response.body().string(),
                        DungeonHubService.getInstance().getCarryInformationSetType());
            } else {
    ...
    public List<ScoreValue> countScore(long serverId, Long id) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.isSuccessful()) {
                if (response.body() != null) {
                    return DungeonHubService.getInstance().getGson().fromJson(response.body().string(),
                            DungeonHubService.getInstance().getScoreValueListType());
                }
            } else {
    ...
    public Map<Long, Long> getLeaderboardData(@NotNull CarryType carryType, ScoreType scoreType, int page) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.isSuccessful()) {
                if (response.body() != null) {
                    result = DungeonHubService.getInstance().getGson().fromJson(response.body().string(),
    ...
    public StrikeData loadStrikeDataFromId(long serverId, long id) throws NotFoundException {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.code() == 404) {
                ...
                if (response.body() != null) {
                    return DungeonHubService.getInstance().getGson().fromJson(response.body().string(), StrikeData.class);
                }
            } else {
